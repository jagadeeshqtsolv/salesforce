...
export class DbQueries {
  async findByEmail(vars: Record<string, string>): Promise<Record<string, string>> {
    const { text } = bindParams("SELECT Lead.FirstName, Lead.LastName, Lead.Salutation, Lead.Company, Lead.Id FROM Lead LIMIT 1", vars, env);
